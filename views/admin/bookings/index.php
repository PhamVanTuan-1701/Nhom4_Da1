        ...
        <table class="table table-hover align-middle">
            ...
            <tbody>
                <?php foreach ($bookings as $b): ?>
                    <?php 
                        // 1. GIẢI MÃ JSON ĐỂ LẤY TÊN VÀ SĐT RIÊNG BIỆT
                        $info = json_decode($b['service_detail'], true);
                        
                        // Lấy tên (Nếu không có thì hiện mặc định)
                        $name = !empty($info['name']) ? $info['name'] : 'Khách vãng lai';
                        
                        // Lấy SĐT
                        $phone = !empty($info['phone']) ? $info['phone'] : 'Không có SĐT';
                        
                        // Tạo avatar dựa trên Tên khách (Mỗi tên sẽ ra 1 avatar khác nhau)
                ...
                <tr>
                    ...
                    <td>
                        <form action="<?= BASE_URL ?>admin/booking/update" method="POST">
                    ...
                    <td>
                        <a href="<?= BASE_URL ?>admin/booking/delete?id=<?= $b['id'] ?>" 
